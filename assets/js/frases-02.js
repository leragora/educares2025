...
const animationConfigs = {
  ...
  'frase01': { 
    containerId: 'item1609', // ID do div onde a animação frase01.json será renderizada
    path: './assets/frase03.json' // Caminho para o arquivo JSON da animação
  },
  // ATENÇÃO: SUBSTITUA 'nome-do-estado-da-frase02' PELO 'name' REAL DO SEU ESTADO NO HTML
  'frase02': { 
    containerId: 'item1632', // ID do div onde a animação frase02.json será renderizada
    path: './assets/frase04.json' // Caminho para o arquivo JSON da animação
  }
...
function handleMultiStateChange() {
  const multiStateElement = document.getElementById('item1553'); 
  if (!multiStateElement) {
    console.warn("MSO com ID 'item1553' não encontrado.");
    return;
  }
...
document.addEventListener('DOMContentLoaded', function() {
  const slideshow = document.getElementById('item1553'); // ATUALIZADO: ID do MSO principal
  if (slideshow) {
